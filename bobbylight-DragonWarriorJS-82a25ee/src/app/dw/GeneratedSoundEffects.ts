type Waveform = 'sine' | 'square' | 'triangle' | 'noise';
...
const BASE_SOUND_RECIPES = new Map<string, SoundRecipe>([
...
const FANTASY_SOUND_RECIPES = new Map<string, SoundRecipe>([
...
const GENERATED_MUSIC_RECIPES = new Map<string, SoundRecipe>([
...
const waveformSample = (waveform: Waveform, phase: number, randomSeed: number): number => {
    switch (waveform) {
        case 'sine':
...
const toLittleEndian = (value: number, bytes: number): number[] => {
    const output: number[] = [];
...
const makeWavDataUri = (samples: Int16Array): string => {
...
const createEnvelope = (index: number, total: number, attack: number, release: number): number => {
...
const generateRecipeSamples = (recipe: SoundRecipe): Int16Array => {
...
const createGeneratedAudioFromRecipes = (recipes: Map<string, SoundRecipe>): Map<string, string> => {
    const output = new Map<string, string>();
    recipes.forEach((recipe: SoundRecipe, key: string) => {
        output.set(key, makeWavDataUri(generateRecipeSamples(recipe)));
    });
    return output;
};

export const createGeneratedSoundEffects = (): Map<string, string> => {
    const output = createGeneratedAudioFromRecipes(BASE_SOUND_RECIPES);
    createGeneratedAudioFromRecipes(FANTASY_SOUND_RECIPES).forEach((uri: string, key: string) => {
        output.set(key, uri);
    });
    return output;
};

export const createGeneratedFantasyMusic = (): Map<string, string> => {
