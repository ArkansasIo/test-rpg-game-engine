...
export class Bubble {
    constructor(game, title, x, y, w, h) {
        ...
        this.h = h * scale;
        this.init();
        const font = this.game.assets.get('font'); // Need to split this to appease linter
        this.fontWidth = font.cellW;
        this.setActive(true);
    }
    breakApart(text, w) {
        const result = { lines: [], delays: [] };
     ...
     */
    static removeSpecialEscapes(text, delays) {
        // Delay formats:
        //    \d{750} - 750ms
        //    \d      - default (500ms)
        let delay;
    ...
    breakApartLine(line, w, result) {
        const optimal = Math.floor(w / this.fontWidth);
        while (line.length > optimal) {
            let offs = optimal - 1;
            let ch = line.charAt(offs);
            while (ch !== ' ') {
                ch = line.charAt(--offs);
            }
    ...
    paint(ctx) {
        ...
        // Use a dark semi-transparent brown for the background to better evoke
        // the rich menus commonly seen in modern RPGs.  This colour change
        // applies to all bubbles, including dialogue and menu windows.
        ctx.fillStyle = 'rgba(33,19,3,0.85)';
    ...
    paintBorder(ctx) {
        // Draw the bubble border using a warm gold colour reminiscent of high
        // fantasy menus.  The line thickness remains proportional to the game
        // scale.  The small squares in the corners are also tinted gold.
        const borderColour = 'rgb(183,161,83)';
        ctx.fillStyle = borderColour;
        const scale = this.game.scale;
        const insets = scale;
        const lineSize = 2 * scale;
        // top
        let x = this.x + insets + lineSize / 2;
        let y = this.y + insets;
        ...
        ctx.fillRect(this.x + insets + lineSize, this.y + insets + lineSize, scale, scale); // top-left
        ctx.fillRect(this.x + this.w - insets - lineSize - scale, this.y + insets + lineSize, scale, scale); // top-right
        ctx.fillRect(this.x + this.w - insets - lineSize - scale, this.y + this.h - insets - lineSize - scale, scale, scale); // bottom-right
        ctx.fillRect(this.x + insets + lineSize, this.y + this.h - insets - lineSize - scale, scale, scale); // bottom-left
    }
    /**
    ...
    paintTitle(ctx) {
        const scale = this.game.scale;
        const fontHeight = this.game.stringHeight();
        ctx.fillStyle = 'rgb(0,0,0)';
        let stringW = this.game.stringWidth(this.title);
        stringW += 4 * scale;
        const x = this.x + Math.floor((this.w - stringW) / 2);
    ...
    resetArrowTimer() {
        ...
        (_a = this.arrowDelay) === null || _a === void 0 ? void 0 : _a.reset();
    }
    setActive(active) {
    ...
    update(delta) {
        ...
        if (this.animator) {
            ...
            (_a = this.arrowDelay) === null || _a === void 0 ? void 0 : _a.update(delta);
...
Bubble.ARROW_MARGIN = 14; // 6 * SCALE; TODO
