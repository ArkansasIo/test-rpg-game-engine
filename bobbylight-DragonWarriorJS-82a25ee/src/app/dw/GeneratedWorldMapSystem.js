...
export function createGeneratedWorldMap(seed) {
    const cols = 180;
    const rows = 96;
    ...
    const tiles = Array.from({ length: rows }, () => Array(cols).fill(WATER_TILE));
...
export function renderGeneratedWorldMapOverlay(ctx, map, heroWorldCol, heroWorldRow, currentMapName) {
    const panelX = 36;
    const panelY = 26;
    const panelW = 650;
    const panelH = 420;
...
//# sourceMappingURL=GeneratedWorldMapSystem.js.map