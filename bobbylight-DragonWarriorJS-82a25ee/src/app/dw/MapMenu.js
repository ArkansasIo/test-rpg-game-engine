...
export class MapMenu {
    ...
    render(ctx) {
        ...
        if (!zones.length) {
            drawFantasyLine(ctx, `Current map: ${(_a = this.mapData.name) !== null && _a !== void 0 ? _a : 'Unknown'}`, panel.bodyX + 8, y, false, 'accent');
        ...
        for (let i = 0; i < zones.length; i++) {
            ...
            const tileData = zoneTiles.find((z) => z.zone === zone.name);
    ...
    handleClick(x, y) {
        ...
        const zones = this.mapData.eldenZones || [];
        if (!zones.length) {
            this.game.setStatusMessage(`Map: ${(_a = this.mapData.name) !== null && _a !== void 0 ? _a : 'Unknown'}`);
            return;
        }
