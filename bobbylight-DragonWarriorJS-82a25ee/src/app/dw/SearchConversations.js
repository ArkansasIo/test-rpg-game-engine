...
import { toLocationString } from '@/app/dw/LocationString';
import { HERB } from '@/app/dw/Item';
/**
 * Returns the text to display when the player opens a treasure chest.
 */
export const getSearchConversation = (state) => {
    const conversation = new Conversation(state.game, false);
    const game = state.game;
    conversation.addSegment('\\w{hero.name} searched the ground all about.');
    const location = toLocationString(game.hero.mapRow, game.hero.mapCol);
    const chest = game.getMap().chests.has(location);
...
//# sourceMappingURL=SearchConversations.js.map