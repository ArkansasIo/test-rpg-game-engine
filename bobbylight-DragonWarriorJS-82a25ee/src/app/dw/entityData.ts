...
export const ELDEN_MONSTERS: Monster[] = eldenMobs.map((mob) => {
    return {
        id: 10000 + mob.id,
        name: mob.name,
        class: 'other',
...
export const ELDEN_WEAPONS: Weapon[] = eldenWeapons.map((weapon) => {
    return {
        id: 10000 + weapon.id,
        name: weapon.name,
        type: 'other',
...
export const ELDEN_ARMORS: Armor[] = eldenArmor.map((armor) => {
    return {
        id: 10000 + armor.id,
...
export const ELDEN_ITEMS: Item[] = eldenKeyItems.map((item) => {
    return {
        id: 10000 + item.id,
        name: item.name,
        type: 'key',
